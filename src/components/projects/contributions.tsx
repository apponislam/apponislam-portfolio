...
import { ProjectContribution } from "@/data/projects2";
        ...
        <TooltipProvider>
            <div className="mt-5 flex flex-wrap gap-4 border-t border-muted pt-4">
                {contributions.map((contributor, idx) => {
                    ...
                    const linkedinUrl = contributor.linkedinUsername
                        ? `https://linkedin.com/in/${contributor.linkedinUsername}`
                        : "#";
                        ...
                        <Tooltip key={idx} delayDuration={100}>
                            <TooltipTrigger asChild>
                                <Link
                                    ...
                                    className="flex items-center space-x-2 text-sm hover:opacity-90 transition-all group p-1.5 rounded-lg hover:bg-accent/40 border border-transparent hover:border-muted"
                                >
                                    <Image
                                        src={profileImg}
                                        ...
                                        width={42}
                                        height={42}
                                        className="rounded-full bg-white object-cover border border-muted group-hover:scale-105 transition-transform"
                                    />
                            ...
                            <TooltipContent
                                side="top"
                                align="start"
                                className="max-w-xs p-3.5 bg-background/95 backdrop-blur-md border border-muted text-foreground shadow-xl rounded-xl"
                            >
                                {hasHighlights ? (
                                    ...
                                    <p className="text-xs text-muted-foreground italic">
                                        Contributed to project architecture & implementation.
                                    </p>
                                )}
                            </TooltipContent>
